...
#include <File.au3>
FileDelete(@UserProfileDir & "\sethopdong.txt")
...
$Button1 = GUICtrlCreateButton("Xong", 192, 316, 75, 25)
GUISetState(@SW_SHOW)
#EndRegion ### END Koda GUI section ###
If FileExists(@UserProfileDir & "\sethopdong.txt") Then
	For $i = 2 To 8 Step 2
...
While 1
	...
	Switch $nMsg
		...
		Case $Button1
			...
			Next
			If $t = 1 Then
				MsgBox(0,'','T với m đã chính thức là bồ nhau.')
				_FileCreate(@UserProfileDir & "\sethopdong.txt")
				Exit
			Else
				MsgBox(0,'','Chưa xong mà láo')
			EndIf
